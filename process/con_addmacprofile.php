<?php
	...
	$login=":delay 2;:local m [ip hotspot active get value-name=mac-address [find user=\$username]];:local date [/system clock get date ];:local time [/system clock get time ];:local uptime (".$uptime.");:if ( [/ip hotspot user get \$user comment ] = \"\" ) do={[/ip hotspot user set mac-address=\$m [find name=\$username]];[/ip hotspot user set \$user comment=[[\$date];[\$time]]];[/system scheduler add disabled=no interval=\$uptime name=\$user on-event= \"[/ip hotspot user remove [find where name=\$user]];[/ip hotspot active remove [find where user=\$user]];[/sys sch re [find where name=\$user]]\" start-date=\$date start-time=\$time]; }";
